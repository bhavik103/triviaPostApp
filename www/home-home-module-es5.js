...
var HomePage = /** @class */ (function () {
    ...
    HomePage.prototype.removeJs = function () {
    ...
    HomePage.prototype.bookmarkedNews = function () {
        ...
        this._newsService.getAllBookmarks().subscribe(function (res) {
            ...
            if (!res.post.length) {
                ...
                _this.isTextVisible = true;
                _this.text = "There are no news yet...";
            }
            _this.newsArray = res.post;
            if (_this.tokenLocalStorage) {
                lodash__WEBPACK_IMPORTED_MODULE_9__["forEach"](_this.newsArray, function (save) {
                    save['bookmarkKey'] = true;
                });
            }
            console.log(_this.newsArray);
            _this.buildForSwiper();
        }, function (err) {
            _this.loading = false;
            _this.error = err;
        });
    ...
    HomePage.prototype.getNews = function () {
        ...
        this._newsService.getAllNews().subscribe(function (res) {
            _this.route.params.subscribe(function (param) {
                if (!_this.router.url.includes('single-news')) {
                    _this.loadNewsToPage(res, userId);
                }
                else {
                    _this.appendSinleNews(res, userId);
                }
            });
        }, function (err) {
            _this.loading = false;
    ...
    HomePage.prototype.loadNewsToPage = function (res, userId) {
        this.newsArray = [];
        this.loading = false;
        if (!res.length) {
            this.isTextVisible = true;
    ...
    HomePage.prototype.bookmark = function (index) {
        ...
        this._newsService.bookmarkPost(this.newsArray[index].newsId).subscribe(function (res) {
            ...
            console.log("bookmarkkey value", index, _this.newsArray[index].bookmarkKey);
    ...
    HomePage.prototype.startCurrentPolling = function () {
        var that = this;
    ...
    HomePage.prototype.newsCount = function (detail) {
        this._newsService.newsCount(detail).subscribe(function (res) {
